...
const getImagesExperiance = (info: string) => {
  if (info !== "") {
    const images =
      "<div>" +
      info
        ...
        .join("") +
      "</div>";
    return `<div class="experiance-images">${images}</div>`;
  } else {
    return `<div class="experiance-images no-image"></div>`;
  }
};
...
export const remarkContainer: RemarkPlugin = () => {
  return (tree) => {
    visit(tree, "containerDirective", (node) => {
      ...
      if (node.name === "experiance") {
        data.hProperties = { class: ["experiance-block"] };
        ...
        const images = getImagesExperiance(info);

        const prev: mdast.HTML = {
          type: "html",
          value: `${images}<div class="experiance-details">`
        };
        const next: mdast.HTML = {
          type: "html",
          value: "</div>"
